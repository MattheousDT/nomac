...
import sunglasses, {
	sunglassesCount,
	sunglassesLeaderboard,
	sunglassesOwn,
	sunglassesOwnage,
} from "./commands/sunglasses";
	...
	client.on("interactionCreate", async (interaction) => {
		...
		switch (interaction.commandName) {
			...
			case "sunglasses":
				switch (interaction.options.getSubcommand()) {
					case "own":
						sunglassesOwn(interaction);
						break;
					case "count":
						sunglassesCount(interaction);
						break;
					case "ownage":
						sunglassesOwnage(interaction);
						break;
					case "leaderboard":
						sunglassesLeaderboard(interaction);
						break;
				}
				break;
		}
...
const main = async () => {
	...
	await rest.put(Routes.applicationGuildCommands(client.application!.id, process.env.SERVER_ID!), {
		body: [abbreviation, fm, sunglasses],
	});
};
